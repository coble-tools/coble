...
conda env remove --name visnetwork -y 2>/dev/null || true
conda create --no-default-packages --name visnetwork -y
...
'r-tidyverse' 
# r-package:
Rscript -e 'install.packages("visNetwork", repos="https://cloud.r-project.org", dependencies=NA, Ncpus=4)'
cat > ${CONDA_PREFIX}/bin/validate.sh << 'VALIDATE_EOF'
#!/usr/bin/env bash
echo "COBLE validation: No script has been specified for visnetwork environment."
VALIDATE_EOF
