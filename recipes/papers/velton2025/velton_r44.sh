...
conda env remove --name velton_r44 -y 2>/dev/null || true
conda create --no-default-packages --name velton_r44 -y
...
# code/coble build --recipe recipes/demos/velton2025/velton_r44.cbl --env velton_r44 --rebuild
#######################################
# comments:
# compilers:
...
echo 'echo "COBLE validation: No script has been specified for velton_r44 environment."' >> ${CONDA_PREFIX}/bin/validate.sh
chmod +x ${CONDA_PREFIX}/bin/validate.sh
chmod +x ${CONDA_PREFIX}/bin/validate.sh
mkdir -p ${CONDA_PREFIX}/coble-recipe
cp recipes/demos/velton2025/velton_r44.cbl ${CONDA_PREFIX}/coble-recipe
