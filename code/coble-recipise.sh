#!/usr/bin/env bash
##############
# Generate the sh script from the recipe
##############
# Inputs ----
# 1. --recipe yamlfile
...
# Clear the aggregate file at the start
{
    ...
    echo -e "if [ -f ~/.bash_profile ]; then source ~/.bash_profile; elif [ -f ~/.bashrc ]; then source ~/.bashrc; elif command -v conda > /dev/null 2>&1; then eval "$(conda shell.bash hook)"; fi"
...
while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ "$line" == "flags:"* \
        ...
        if [[ "$line" == "validate:"* ]]; then
            echo "# Validation section - creating and copying the validation to CONDA_PREFIX" >> "$RECIPE_FILE"
